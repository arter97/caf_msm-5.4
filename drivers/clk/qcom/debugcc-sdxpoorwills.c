...
static int gcc_debug_mux_sels[] = {
	...
	0x49,		/* gcc_prng_ahb_clk */
	0x33,		/* gcc_sdcc1_ahb_clk */
	0x32,		/* gcc_sdcc1_apps_clk */
	0xB5,		/* gcc_spmi_fetcher_ahb_clk */
	0xB4,		/* gcc_spmi_fetcher_clk */
	0xB,		/* gcc_sys_noc_usb3_clk */
...
static struct clk_hw *debugcc_sdxpoorwills_hws[] = {
...
static int clk_debug_sdxpoorwills_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_sdxpoorwills_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_sdxpoorwills_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%d)\n",
				clk_hw_get_name(debugcc_sdxpoorwills_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
