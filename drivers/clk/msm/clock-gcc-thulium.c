...
static struct rcg_clk usb30_master_clk_src = {
	...
	.c = {
		...
		.ops = &clk_ops_rcg_mnd,
		VDD_DIG_FMAX_MAP2(LOWER, 60000000, LOW, 125000000),
...
static struct of_device_id msm_clock_gcc_match_table[] = {
	{ .compatible = "qcom,gcc-thulium" },
	{}
};

static struct platform_driver msm_clock_gcc_driver = {
	.probe = msm_gcc_thulium_probe,
	.driver = {
		.name = "qcom,gcc-thulium",
		.of_match_table = msm_clock_gcc_match_table,
		.owner = THIS_MODULE,
	},
};

int __init msm_gcc_thulium_init(void)
{
	return platform_driver_register(&msm_clock_gcc_driver);
}
arch_initcall(msm_gcc_thulium_init);
...
static struct of_device_id msm_clock_debug_match_table[] = {
	{ .compatible = "qcom,cc-debug-thulium" },
	{}
};

static int msm_clock_debug_thulium_probe(struct platform_device *pdev)
{
	struct resource *res;
	int ret;
	...
	ret = of_msm_clock_register(pdev->dev.of_node,
				    ...
				    ARRAY_SIZE(msm_clocks_measure_thulium));
	if (ret)
		return ret;
...
static struct platform_driver msm_clock_debug_driver = {
	.probe = msm_clock_debug_thulium_probe,
	.driver = {
		.name = "qcom,cc-debug-thulium",
		.of_match_table = msm_clock_debug_match_table,
		.owner = THIS_MODULE,
	},
};

int __init msm_clock_debug_thulium_init(void)
{
	return platform_driver_register(&msm_clock_debug_driver);
}
late_initcall(msm_clock_debug_thulium_init);
