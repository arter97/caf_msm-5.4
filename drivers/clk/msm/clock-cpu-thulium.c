...
static int cpu_mux_set_sel(struct mux_clk *mux, int sel)
{
	 ...
	 * is disabled (specifically on thulium V1, the LFMUX may be
	 * disabled).
	 */
...
struct cpu_clk_thulium {
	u32 cpu_reg_mask;
	struct clk c;
};

static inline struct cpu_clk_thulium *to_cpu_clk_thulium(struct clk *c)
{
	return container_of(c, struct cpu_clk_thulium, c);
}

static enum handoff cpu_clk_thulium_handoff(struct clk *c)
{
...
static long cpu_clk_thulium_round_rate(struct clk *c, unsigned long rate)
{
	return clk_round_rate(c->parent, rate);
}

static int cpu_clk_thulium_set_rate(struct clk *c, unsigned long rate)
{
	return clk_set_rate(c->parent, rate);
}

static struct clk_ops clk_ops_cpu_thulium = {
	.set_rate = cpu_clk_thulium_set_rate,
	.round_rate = cpu_clk_thulium_round_rate,
	.handoff = cpu_clk_thulium_handoff,
};

DEFINE_VDD_REGS_INIT(vdd_pwrcl, 1);

static struct cpu_clk_thulium pwrcl_clk = {
	.cpu_reg_mask = 0x3,
	.c = {
		.parent = &pwrcl_hf_mux.c,
		.dbg_name = "pwrcl_clk",
		.ops = &clk_ops_cpu_thulium,
		.vdd_class = &vdd_pwrcl,
...
static struct cpu_clk_thulium perfcl_clk = {
	.cpu_reg_mask = 0x103,
	.c = {
...
static struct clk_lookup cpu_clocks_thulium[] = {
	CLK_LIST(pwrcl_clk),
	CLK_LIST(pwrcl_pll),
...
static int cpu_clock_thulium_resources_init(struct platform_device *pdev)
{
	struct resource *res;
	struct clk *c;
	int i;
...
static void populate_opp_table(struct platform_device *pdev)
{
	...
	pr_info("clock-cpu-thulium: OPP tables populated.\n");
}

static int perfclspeedbin;

static int cpu_clock_thulium_driver_probe(struct platform_device *pdev)
{
	int ret, cpu;
	...
	ret = cpu_clock_thulium_resources_init(pdev);
	if (ret) {
	...
	ret = of_msm_clock_register(pdev->dev.of_node, cpu_clocks_thulium,
				    ARRAY_SIZE(cpu_clocks_thulium));
	if (ret) {
...
static struct of_device_id match_table[] = {
	{ .compatible = "qcom,cpu-clock-thulium" },
	{}
};

static struct platform_driver cpu_clock_thulium_driver = {
	.probe = cpu_clock_thulium_driver_probe,
	.driver = {
		.name = "cpu-clock-thulium",
		.of_match_table = match_table,
		.owner = THIS_MODULE,
	},
};

static int __init cpu_clock_thulium_init(void)
{
	return platform_driver_register(&cpu_clock_thulium_driver);
}
arch_initcall(cpu_clock_thulium_init);

static void __exit cpu_clock_thulium_exit(void)
{
	platform_driver_unregister(&cpu_clock_thulium_driver);
}
module_exit(cpu_clock_thulium_exit);
...
early_initcall(cpu_clock_thulium_init_perfcl);

MODULE_DESCRIPTION("CPU clock driver for msmthulium");
