...
const static uint32_t g_client_version = 1;
	...
	const char *metainfo)
{
	...
	strlcpy(rpc_params.metainfo, metainfo, sizeof(rpc_params.metainfo));
				...
				struct hgsl_mem_node *mem_node)
{
	...
	ret = habmm_export(hab_channel->socket, hab_exp_handle, PAGE_SIZE,
				&export_id, HABMM_EXPIMP_FLAGS_FD);
	if (ret) {
				...
				struct hgsl_mem_node *mem_node)
{
	...
	if (rval == GSL_SUCCESS) {
		...
		ctxt->shadow_ts = (struct shadow_ts *)dma_buf_vmap(mem_node->dma_buf);
		if (ctxt->shadow_ts == NULL) {
			...
			int dbq_off, uint32_t *dbq_info)
{
	...
	if (ret != 0) {
		read_shadowts_mem_be(hab_channel, ctxt);
		ret = 0;
	}
