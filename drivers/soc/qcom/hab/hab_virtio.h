...
struct virtio_hab {
	...
	int vqs_offset; // in case vqs are not start from zero to support all the needs of one virtio device, and it always starts after "other"  vqs 
...
// this commdev has two parts, the pchan for hab driver created in commdev alloc, and,
// virtio dev and vqs created during virtio probe. commdev might happen earlier than virtio probe
// one kind of hab driver for one kind of virtio device. within this one pair there is one list/array
// of pchans/commdevs
...
int virthab_alloc(struct virtio_device *vdev, struct virtio_hab **pvh, uint32_t mmid_start, int mmid_range);
