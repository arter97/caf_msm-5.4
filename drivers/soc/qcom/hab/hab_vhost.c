	...
	while (1) {
		...
		if ((out_num > 0) && (out_len > 0)) {
			...
			if (unlikely(copy_size != sizeof(header)))
				pr_err("fault on copy_from_iter, out_len %lu, "
					"ret %lu\n", out_len, copy_size);
		...
		if (in_num) {
			...
			total_len += in_len;
			pr_warn("unexpected in buf in tx vq, return directly, "
				"in_num %d, in_len %lu\n", in_num, in_len);
		}
...
static int vhost_hab_open(struct inode *inode, struct file *f)
{
	...
	pr_info("vhost_hab_open: vh_cdev=%px\n", vh_cdev);
	...
	vqs = kmalloc(num_pchan * VHOST_HAB_PCHAN_VQ_MAX * sizeof(*vqs),
								GFP_KERNEL);
#else
	...
	vhost_dev_init(&vh_dev->dev, vqs, VHOST_HAB_PCHAN_VQ_MAX * num_pchan,
			...
			UIO_MAXIOV,
#endif
			VHOST_HAB_PKT_WEIGHT, VHOST_HAB_WEIGHT);
...
static int vhost_hab_release(struct inode *inode, struct file *f)
{
	...
	vhost_dev_cleanup(&vh_dev->dev, false);
#else
	vhost_dev_stop(&vh_dev->dev);
...
static long vhost_hab_run(struct vhost_hab_dev *vh_dev, int start)
{
	...
	if (not_started == 0) {
		...
		pr_info("exit vh_dev %px start %d r %d ret %d not_started %d\n",
			vh_dev, start, r, ret, not_started);
		return 0;
	}
exit:
	mutex_unlock(&vh_dev->dev.mutex);
	pr_info("exit vh_dev %px start %d failure r %d ret %d not_started %d\n",
		vh_dev, start, r, ret, not_started);
	return r;
}
				...
				struct vhost_config *cfg)
{
	...
	size_t cfg_size = min((size_t)cfg->size, sizeof(hab_cfg));
	char *s, *t;
	int vmid;
	int ret;
	...
	pr_info("vhost_hab_set_config: vm_name %s\n", hab_cfg.vm_name);
				...
				unsigned long arg)
{
	...
	default:
	{
		...
		vhost_hab_flush(vh_dev);
		mutex_unlock(&vh_dev->dev.mutex);
		if (0 == vhost_hab_ready_check(vh_dev))
			...
			size_t *out_len, int *head)
{
	...
	while (1) {
		*head = vhost_get_vq_desc(vq, vq->iov, ARRAY_SIZE(vq->iov),
		...
		if (*head == vq->num) {
			pr_info("rx buf %s underrun, vq avail %d\n",
				vh_pchan->pchan->name,
				vhost_vq_avail_empty(dev, vq));

			if (unlikely(vhost_enable_notify(dev, vq))) {
				pr_info("%s enable notify return true, %d\n",
					vh_pchan->pchan->name,
					vhost_vq_avail_empty(dev, vq));
			...
			*in_len = iov_length(&vq->iov[out_num], in_num);
			if (*in_len > 0) {
				iov_iter_init(in_iter, READ, &vq->iov[out_num],
						in_num, *in_len);
				break; /* only one send goes out */
			} else {
				pr_err("out of in-len in iov %d "
					"in-num %d out-num %d\n",
			...
			pr_err("no in buff need next round!\n");
			ret = -EBUSY;
			break;
		}
	}
				   ...
				   int *added)
{
	...
	while (remain_size > 0) {
		...
		if (in_len) {
			...
			if (ret) {
				pr_err("%s failed to add used ret %d "
					"head %d size %d\n",
					vh_pchan->pchan->name, ret, head,
					size_filled);
				break;
			}
			*added += 1; /* continue for the remaining */
		} else {
			pr_err("%s rx-buf empty ret %d inlen %d outlen %d "
				"head %d\n",
				vh_pchan->pchan->name, ret, in_len, out_len,
				head);
			ret = -EPIPE;
			break;
		}
...
static int rx_worker(struct vhost_hab_pchannel *vh_pchan)
{
	struct vhost_hab_send_node *send_node, n;
	struct vhost_virtqueue *vq = &vh_pchan->vqs[VHOST_HAB_PCHAN_RX_VQ];
			...
			void* payload)
{
	...
	send_node = kmalloc(sizebytes + sizeof(struct vhost_hab_send_node),
				GFP_KERNEL);
	if (!send_node) {
		pr_err("failed to allocate send memory\n");
		return -ENOMEM;
	}
...
struct file_operations vhost_hab_fops = {
	.owner = THIS_MODULE,
	.open = vhost_hab_open,
			...
			int vmid_remote, struct hab_device *habdev)
{
	...
	if (!pchan) {
		pr_err("failed to create %s pchan in mmid device %s, "
			"pchan cnt %d\n", name, habdev->name,
			habdev->pchan_cnt);
		*commdev = NULL;
		return -ENOMEM;
	...
	if (habdev->pchan_cnt == 1) {
		...
		if (ret) {
			pr_err("vhost_hab_cdev_add_hab_device failed, "
	...
	*commdev = pchan;
	...
	pr_info("pchan %s vchans %d refcnt %d vh-pchan %px\n",
