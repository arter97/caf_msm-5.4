...
static struct virtio_hab *g_hab_virtio = NULL; 
...
int store_vdev(struct virtio_device *vdev, struct virtio_hab *vh)
{
		...
		while ((hd = (struct vh_buf_header *)virtqueue_get_buf(vq, &len)) != NULL) {
			...
			case PT_OUT_SMALL:
				if ((hd->index > OUT_SMALL_BUF_NUM) ||
					...
					pr_err("small buf index corrupted "
						"%pK %pK %d %d\n",
						hd, hd->buf, hd->index,
						hd->size);
			...
			case PT_OUT_MEDIUM:
				if ((hd->index > OUT_MEDIUM_BUF_NUM) ||
					...
					pr_err("medium buf index corrupted "
						"%pK %pK %d %d\n",
						hd, hd->buf, hd->index,
						hd->size);
			...
			case PT_OUT_LARGE:
				if ((hd->index > OUT_MEDIUM_BUF_NUM) ||
					...
					pr_err("large buf index corrupted "
						"%pK %pK %d %d\n",
						hd, hd->buf, hd->index,
						hd->size);
...
static void virthab_recv_rxq(struct virtqueue *vq)
{
	struct virtio_hab *vh = get_vh(vq->vdev);
	struct vq_pchan *vpc = get_virtio_pchan(vh, vq);
	char *inbuf;
	...
	spin_lock_irqsave(&vpc->lock[HAB_PCHAN_RX_VQ], flags);
	...
	while ((hd = virtqueue_get_buf(vpc->vq[HAB_PCHAN_RX_VQ], &len)) != NULL) {
		...
		if (!pchan)
			...
			spin_unlock_irqrestore(&vpc->lock[HAB_PCHAN_RX_VQ],
						flags);
	...
	spin_unlock_irqrestore(&vpc->lock[HAB_PCHAN_RX_VQ], flags);
}
	...
	for (i = 0; i < vh->mmid_range; i++) {
		...
		if (alloc) {
			...
			if (!vpc->in_pool || !vpc->s_pool || !vpc->m_pool ||
				!vpc->l_pool) {
				pr_err("failed to allocate buffer %d %pK %d "
					"%pK %d %pK %d %pK\n",
			...
			pr_debug("hab VQ buf allocated %s %d %d %d %d %d %d "
				"%d %d %pK %pK %pK %pK\n",
		...
		list_for_each_entry_safe(hd, hd_tmp, &vpc->in_list, node) {
			...
			ret = virtqueue_add_inbuf(vpc->vq[HAB_PCHAN_RX_VQ], sg,
							1, hd, GFP_ATOMIC);
			if (ret) {
				pr_err("failed to queue %s inbuf %d "
					"to PVM %d\n",
					vpc->vq[HAB_PCHAN_RX_VQ]->name,
					vpc->in_cnt, ret);
			}
...
EXPORT_SYMBOL_GPL(virthab_queue_inbufs);
...
int virthab_init_vqs_pre(struct virtio_hab *vh)
{
	...
	for (i = 0; i < vh->mmid_range; i++) {
		...
		cbs[i * HAB_PCHAN_VQ_MAX + HAB_PCHAN_RX_VQ] = virthab_recv_rxq;
...
EXPORT_SYMBOL_GPL(virthab_init_vqs_pre);
...
int virthab_init_vqs_post(struct virtio_hab *vh)
{
	...
	for (i = 0; i < vh->mmid_range; i++) {
		...
		vpchans[i].pchan_ready = 1;
		link = (struct virtio_pchan_link *)vpchans[i].pchan->hyp_data;
		link->vpc = &vpchans[i];
		link->vhab = vh;
		pr_info("mmid %d slot %d vhab %pK vdev %pK vq tx %pK "
			"pchan %pK\n",
...
EXPORT_SYMBOL_GPL(virthab_init_vqs_post);
...
static int virthab_init_vqs(struct virtio_hab *vh)
{
	...
	ret = vh->vdev->config->find_vqs(vh->vdev,
					hab_driver.ndevices * HAB_PCHAN_VQ_MAX,
					vh->vqs, cbs,
			...
			uint32_t mmid_start, int mmid_range)
{
	...
	if (!vh->vqs) {
		pr_err("OutOfMemory %zd bytes for virtio hab vqs\n",
			   sizeof(vh->vqs[0]) * mmid_range);
		return -ENOMEM;
	}
	...
	vh->cbs = kzalloc(sizeof(vq_callback_t *) * mmid_range *
				HAB_PCHAN_VQ_MAX, GFP_KERNEL);
	if (!vh->vqs) {
		pr_err("OutOfMemory %zd bytes for virtio hab cbs\n",
			   sizeof(vh->cbs[0]) * mmid_range);
		return -ENOMEM;
	}
	...
	vh->names = kzalloc(sizeof(char *) * mmid_range *
				HAB_PCHAN_VQ_MAX, GFP_KERNEL);
	if (!vh->names) {
		pr_err("OutOfMemory %zd bytes for names\n",
			   sizeof(char *) * mmid_range * HAB_PCHAN_VQ_MAX);
		return -ENOMEM;
	}

	vh->vqpchans = kzalloc(sizeof(struct vq_pchan) * hab_driver.ndevices,
				GFP_KERNEL);
	if (!vh->vqpchans) {
		pr_err("OutOfMemory %zd bytes for vq pchans\n",
			sizeof(struct vq_pchan) * mmid_range *
			HAB_PCHAN_VQ_MAX);
		return -ENOMEM;
	}
	...
	for (i = 0; i < mmid_range * HAB_PCHAN_VQ_MAX; i++) {
		...
		if (!vh->names[i]) {
			pr_err("OutOfMemory %d bytes for names[%d]\n",
				   MAX_VMID_NAME_SIZE + 2, i);
			return -ENOMEM;
		}
	}
				  ...
				  uint32_t mmid_start, int mmid_range)
{
	...
	if (!vh) {
		pr_err("OutOfMemory %zd bytes for virtio hab\n", sizeof(*vh));
		return -ENOMEM;
	}
...
EXPORT_SYMBOL_GPL(virthab_alloc);
						...
						size_t read_size)
{
	...
	if (!payload || !vpc->read_data) {
		pr_err("%s invalid parameters %pK %pK offset %d "
			"read %zd %s dev %pK\n", pchan->name,
			payload, vpc->read_data, vpc->read_offset,
	...
	if (vpc->read_size < read_size + sizeof(struct hab_header) +
		vpc->read_offset) {
		pr_warn("%s read %zd is less than requested %zd header %zd "
			"offset %d\n",
...
static int habvirtio_pchan_create(struct hab_device *dev, char* pchan_name)
{
	...
	if (!link) {
		pr_err("failed to allocate %zd bytes for commdev\n",
					sizeof(*link));
		result = -ENOMEM;
		goto err;
	}
	...
	if (unlikely(g_hab_virtio)) {
		pr_warn("virtio device has been initialized already gvi "
		...
		if (!bfound)
			pr_err("can't find virtio pchan object %s in vhab %pK "
				"total %d\n",
		...
		pr_warn("virtio device has NOT been initialized yet. "
			"%s has to wait for probe\n", pchan->name);
	}
				...
				struct hab_device *mmid_device)
{
	...
	if (ret) {
		pr_err("failed to create %s pchan in mmid device %s, ret %d, "
			"pchan cnt %d\n", name, mmid_device->name, ret,
			mmid_device->pchan_cnt);
		*commdev = NULL;
		return ret;
	}

	pr_info("create virtio pchan on %s return %d, loopback mode(%d), "
