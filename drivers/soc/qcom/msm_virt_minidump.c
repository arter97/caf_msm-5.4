...
static inline int md_region_num(const char *name, int *seqno)
{
	...
	regno = readl_relaxed(md_base + MD_SS_TOC_REGION_COUNT);
...
static void md_update_ss_toc(const struct md_region *entry)
{
	...
	reg_cnt = readl_relaxed(md_base + MD_SS_TOC_REGION_COUNT);
	...
	writel_relaxed(reg_cnt + 1, md_base + MD_SS_TOC_REGION_COUNT);
}

bool msm_minidump_enabled(void)
{
	bool ret = false;
	u32 status = MD_SS_DISABLED;

	spin_lock(&mdt_lock);
	if (md_base)
		status = readl_relaxed(md_base + MD_SS_TOC_ENABLE_STATUS);
	if (status == MD_SS_ENABLED)
		ret = true;
...
int msm_minidump_remove_region(const struct md_region *entry)
{
	...
	if (!entry || status != MD_SS_ENABLED)
		return -EINVAL;

	spin_lock(&mdt_lock);
	ecount = minidump_table.num_regions;
	rcount = readl_relaxed(md_base + MD_SS_TOC_REGION_COUNT);
	...
	writel_relaxed(0, md_base + MD_SS_TOC_INIT);
	...
	writel_relaxed(rcount - 1, md_base + MD_SS_TOC_REGION_COUNT);
	writel_relaxed(1, md_base + MD_SS_TOC_INIT);
...
static int msm_virt_minidump_probe(struct platform_device *pdev)
{
	unsigned int i;
	struct md_region *mdr;
	struct md_global_toc *md_global_toc = NULL;

	md_base = devm_of_iomap(&pdev->dev, pdev->dev.of_node, 0, NULL);
	if (!md_base) {
	...
	md_global_toc->md_toc_init =
		readl_relaxed(md_base + MD_GLOBAL_TOC_INIT);
	if (!md_global_toc->md_toc_init) {
		dev_err(&pdev->dev, "System Minidump TOC not initialized\n");
		return -ENODEV;
	}

	md_global_toc->md_revision =
		readl_relaxed(md_base + MD_GLOBAL_TOC_REVISION);
	md_global_toc->md_enable_status = readl_relaxed(md_base +
			MD_GLOBAL_TOC_ENABLE_STATUS);

	minidump_table.md_gbl_toc = md_global_toc;
	minidump_table.revision = md_global_toc->md_revision;

	writel_relaxed(MD_SS_ENCR_NONE, md_base + MD_SS_TOC_ENCRYPTION_STATUS);
	writel_relaxed(MD_SS_ENCR_REQ, md_base + MD_SS_TOC_ENCRYPTION_REQUIRED);
	...
	writel_relaxed((u32)virt_to_phys(minidump_table.md_regions),
			md_base + MD_SS_TOC_REGION_BASE_LOW);
	writel_relaxed((u32)(virt_to_phys(minidump_table.md_regions) >> 32),
			md_base + MD_SS_TOC_REGION_BASE_HIGH);

	/* First entry would be ELF header */
	writel_relaxed(1, md_base + MD_SS_TOC_REGION_COUNT);
	msm_minidump_add_header();

	/* Add pending entries to HLOS TOC */
	spin_lock(&mdt_lock);
	writel_relaxed(1, md_base + MD_SS_TOC_INIT);
	writel_relaxed(MD_SS_ENABLED, md_base + MD_SS_TOC_ENABLE_STATUS);
...
static const struct of_device_id msm_virt_minidump_device_tbl[] = {
	{ .compatible = "qcom,virt-minidump", },
	{},
};

static struct platform_driver msm_virt_minidump_driver = {
	.probe = msm_virt_minidump_probe,
	.driver = {
		.name = "msm_virt_minidump",
		.of_match_table = msm_virt_minidump_device_tbl,
	},
};

static int __init msm_virt_minidump_init(void)
{
	return platform_driver_register(&msm_virt_minidump_driver);
}
subsys_initcall(msm_virt_minidump_init);

static void __exit msm_virt_minidump_exit(void)
{
	platform_driver_unregister(&msm_virt_minidump_driver);
}
module_exit(msm_virt_minidump_exit);

MODULE_DESCRIPTION("QTI virtual minidump driver");
