...
static unsigned int lt8912_i2c_read_byte(struct lt8912_data *data, unsigned int reg)
{
	int rc = 0;
	int val = 0;
...
static int lt8912_i2c_write_byte(struct lt8912_data *data, unsigned int reg, unsigned int val)
{
	int rc = 0;
	rc = regmap_write(data->regmap, reg, val);
	if (rc) {
		dev_err(&data->lt8912_client->dev, "write 0x%x register failed\n",
				reg);
		return rc;
	}
...
static void lt8912_process_data(struct lt8912_data *data)
{
	...
	if ( val != data->last_val ) {
		...
		input_event(data->input_hotplug, EV_SYN, SYN_TIME_SEC,
					ktime_to_timespec(timestamp).tv_sec);
		input_event(data->input_hotplug, EV_SYN, SYN_TIME_NSEC,
					ktime_to_timespec(timestamp).tv_nsec);
		input_sync(data->input_hotplug);

		dev_dbg(&data->lt8912_client->dev, "input report val = %d\n", val);
	}
...
static int dds_config(struct lt8912_data *data)
{
	...
	/* [0]=strm_sw_freq_word[24]//[7]=strm_sw_freq_word_en=0, [6]=strm_err_clr=0 */
...
static int lt8912_init_input(struct lt8912_data *data)
{
	...
	if (!input) {
		dev_err(&data->lt8912_client->dev, "allocate light input device failed\n");
	...
	if (status) {
		dev_err(&data->lt8912_client->dev, "register light input device failed.\n");
		return status;
	}
...
static int __init lontium_i2c_test_init(void)
{
	...
	if(ret < 0) {
		printk("lontium register i2c failed\n");
		return -1;
	} else {
		printk("probe lt8912 success\n");
		return ret;
	}
}
