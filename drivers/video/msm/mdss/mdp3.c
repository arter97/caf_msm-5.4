...
struct reg_bus_client *mdp3_bus_vote_client_create(char *client_name)
{
	...
	mutex_lock(&mdp3_res->reg_bus_lock);
	...
	pr_debug("bus vote client %s created:%p id :%d\n", client_name,
		client, id);
	id++;
	list_add(&client->list, &mdp3_res->reg_bus_clist);
	mutex_unlock(&mdp3_res->reg_bus_lock);
...
static int mdp3_probe(struct platform_device *pdev)
{
	...
	mdp3_res->mdss_util->bus_vote_client_create = mdp3_bus_vote_client_create;
