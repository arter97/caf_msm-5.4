		...
		u16 reg, int bytes, void *src, u8 pgd)
{
	...
	while (1) {
		...
		ret = slim_change_val_element( pgd ? btfmslim->slim_pgd:&btfmslim->slim_ifd,
			&msg, src, bytes);
	...
	for (i = 0; i < bytes; i++)
		BTFMSLIM_DBG("Write 0x%02x to reg 0x%x", ((u8 *)src)[i], reg + i);
	return ret;
}

int btfm_slim_write_pgd(struct btfmslim *btfmslim,
		u16 reg, int bytes, void *src)
{
				...
				int bytes, void *dest, u8 pgd)
{
	...
	while (1) {
		mutex_lock(&btfmslim->xfer_lock);
		ret = slim_request_val_element(pgd ? btfmslim->slim_pgd:&btfmslim->slim_ifd,
			       &msg, dest, bytes);
	...
	for (i = 0; i < bytes; i++)
		BTFMSLIM_DBG("Read 0x%02x from reg 0x%x", ((u8 *)dest)[i], reg + i);

	return ret;
}

int btfm_slim_read_pgd(struct btfmslim *btfmslim,
		u16 reg, int bytes, void *dest)
{
	...
	u8 rxport, u32 rates, u8 grp, u8 nchan)
{
	...
	if(!btfmslim || !ch) {
		BTFMSLIM_ERR("btfmslim is %p, ch is %p", btfmslim, ch);
		return -EINVAL;
	}
	...
	prop.dataf = (rates == 48000) ? SLIM_CH_DATAF_NOT_DEFINED : SLIM_CH_DATAF_LPCM_AUDIO;
	...
	ret = slim_define_ch(btfmslim->slim_pgd, &prop, ch_h, nchan, grp, &ch->grph);
	if (ret < 0) {
	...
	for(i=0;i<nchan; i++, ch++) {
		...
		if(rxport) {
			BTFMSLIM_INFO("slim_connect_sink(port: %d, ch: %d)", ch->port, ch->ch);
			/* Connect Port with channel given by Machine driver*/
			ret = slim_connect_sink(btfmslim->slim_pgd, &ch->port_hdl, 1, ch->ch_hdl);
			if (ret < 0) {
				BTFMSLIM_ERR("slim_connect_sink failed ret[%d]", ret);
				goto remove_channel;
			}

		} else {
			BTFMSLIM_INFO("slim_connect_src(port: %d, ch: %d)", ch->port, ch->ch);
			/* Connect Port with channel given by Machine driver*/
			ret = slim_connect_src(btfmslim->slim_pgd, ch->port_hdl, ch->ch_hdl);
			if (ret < 0) {
				BTFMSLIM_ERR("slim_connect_src failed ret[%d]", ret);
				goto remove_channel;
			}
		}
	}

	/* Set back to first channel */
	for(i=0; i < nchan; i++, ch--);
	/* Activate the channel immediately */
         BTFMSLIM_INFO("slim_control_ch(port: %d, ch: %d, grp: %d, ch->grph: 0x%x, ch_hdl: 0x%x)",
                ch->port, ch->ch, grp, ch->grph, ch->ch_hdl );
	...
	u8 rxport, u8 grp, u8 nchan )
{
	...
	if (ret < 0) {
		BTFMSLIM_ERR("slim_control_ch failed ret[%d]", ret);
		ret = slim_disconnect_ports(btfmslim->slim_pgd, &ch->port_hdl, 1);
		if (ret < 0) {
			BTFMSLIM_ERR("slim_disconnect_ports failed ret[%d]", ret);
			goto error;
		}
	}
	...
	for(i =0; i< nchan; i++, ch++) {
		if(btfmslim->vendor_port_en) {
			ret = btfmslim->vendor_port_en(btfmslim, ch->port, rxport, 0);
			if (ret < 0) {
				BTFMSLIM_ERR("vendor_port_en failed ret[%d]", ret);
				goto error;
			}
...
static int btfm_slim_alloc_port(struct btfmslim *btfmslim)
{
	...
	BTFMSLIM_INFO("Rx: id\tname\tport\thdl\tch\tch_hdl");
	for ( i=0 ; (rx_chs->port != BTFM_SLIM_PGD_PORT_LAST)&& (i < BTFM_SLIM_NUM_CODEC_DAIS);
		i++, rx_chs++) {
		/* Get Rx port handler from slimbus driver based on port number */
		ret = slim_get_slaveport(btfmslim->slim_pgd->laddr, rx_chs->port,
			&rx_chs->port_hdl, SLIM_SINK);
		if (ret < 0) {
			BTFMSLIM_ERR("slave port failure port#%d - ret[%d]", rx_chs->port, SLIM_SINK);
			goto error;
		}
		BTFMSLIM_INFO("    %d\t%s\t%d\t%x\t%d\t%x", rx_chs->id, rx_chs->name, rx_chs->port,
			rx_chs->port_hdl, rx_chs->ch, rx_chs->ch_hdl);
	}

	BTFMSLIM_INFO("Tx: id\tname\tport\thdl\tch\tch_hdl");
	for ( i=0 ; (tx_chs->port != BTFM_SLIM_PGD_PORT_LAST)&& (i < BTFM_SLIM_NUM_CODEC_DAIS);
		i++, tx_chs++) {
		/* Get Tx port handler from slimbus driver based on port number */
		ret = slim_get_slaveport(btfmslim->slim_pgd->laddr, tx_chs->port,
			&tx_chs->port_hdl, SLIM_SRC);
		if (ret < 0) {
			BTFMSLIM_ERR("slave port failure port#%d - ret[%d]", tx_chs->port, SLIM_SRC);
			goto error;
		}
		BTFMSLIM_INFO("    %d\t%s\t%d\t%x\t%d\t%x", tx_chs->id, tx_chs->name, tx_chs->port,
			tx_chs->port_hdl, tx_chs->ch, tx_chs->ch_hdl);
	}
error:
	return ret;
}
...
int btfm_slim_hw_init(struct btfmslim *btfmslim)
{
	...
	if(btfmslim->enabled) {
		BTFMSLIM_INFO("Already enabled");
		return 0;
	}
	mutex_lock(&btfmslim->io_lock);

	/* Assign Logical Address for PGD (Ported Generic Device) enumeration address */
	...
	/* Assign Logical Address for Ported Generic Device enumeration address */
	...
	/* Allocate ports with logical address to get port handler from slimbus driver */
	ret = btfm_slim_alloc_port(btfmslim);
	if (ret) {
		BTFMSLIM_ERR("failed to allocate port: %d",ret);
		goto error;
	}
	...
	/* Only when all registers read/write successfully, it set to enabled status */
	btfmslim->enabled = 1;
error:
...
int btfm_slim_hw_deinit(struct btfmslim *btfmslim)
{
	...
	if(!btfmslim->enabled) {
		BTFMSLIM_INFO("Already disabled");
		return 0;
	}
	mutex_lock(&btfmslim->io_lock);
	btfmslim->enabled = 0;
...
static int btfm_slim_get_dt_info(struct btfmslim *btfmslim)
{
	...
	if(!slim || !slim_ifd) {
		BTFMSLIM_ERR("slim is %p, slim_ifd is %p", slim, slim_ifd);
		return -EINVAL;
	}

	if (slim->dev.of_node) {
		BTFMSLIM_INFO("Platform data from device tree (%s)", slim->name );
		ret = of_property_read_string(slim->dev.of_node, "qcom,btfm-slim-ifd",
						  &slim_ifd->name);
		if (ret) {
			BTFMSLIM_ERR( "Looking up %s property in node %s failed",
				"qcom,btfm-slim-ifd", slim->dev.of_node->full_name);
			return -ENODEV;
		}
		BTFMSLIM_INFO("qcom,btfm-slim-ifd (%s)", slim_ifd->name );
		...
		if (!prop) {
			...
			BTFMSLIM_ERR("invalid codec slim ifd addr. addr length = %d",
					  prop->length);
			return -ENODEV;
		}
		memcpy(slim_ifd->e_addr, prop->value, 6);
		BTFMSLIM_DBG("PGD Enum Addr: %.02x:%.02x:%.02x:%.02x:%.02x:%.02x",
			slim->e_addr[0], slim->e_addr[1], slim->e_addr[2],
			slim->e_addr[3], slim->e_addr[4], slim->e_addr[5] );
		BTFMSLIM_DBG("IFD Enum Addr: %.02x:%.02x:%.02x:%.02x:%.02x:%.02x",
			slim_ifd->e_addr[0], slim_ifd->e_addr[1], slim_ifd->e_addr[2],
			slim_ifd->e_addr[3], slim_ifd->e_addr[4], slim_ifd->e_addr[5] );
...
struct btfmslim * btfm_slim_get_data(void )
{
	return btfm_slim;
}
...
static int btfm_slim_probe(struct slim_device *slim)
{
	...
	if (!slim->ctrl) {
		BTFMSLIM_ERR("Error, no SLIMBUS control data");
		return -EINVAL;
	}
...
static int btfm_slim_remove(struct slim_device *pdev)
{
	BTFMSLIM_DBG("");
	mutex_destroy(&btfm_slim->io_lock);
	mutex_destroy(&btfm_slim->xfer_lock);
	...
	slim_remove_device(pdev);
	return 0;
}

static int btfm_slim_device_up(struct slim_device *sldev)
{
	BTFMSLIM_DBG("Not yet suported");
	return 0;
}

static int btfm_slim_device_reset(struct slim_device *sldev)
{
	BTFMSLIM_DBG("Not yet suported");
	return 0;
}

static int btfm_slim_device_down(struct slim_device *sldev)
{
	BTFMSLIM_DBG("Not yet suported");
	return 0;
}

static int btfm_slim_resume(struct slim_device *sldev)
{
	BTFMSLIM_DBG("Not yet suported");
	return 0;
}

static int btfm_slim_suspend(struct slim_device *sldev, pm_message_t pmesg)
{
	BTFMSLIM_DBG("Not yet suported");
	return 0;
}
...
static struct slim_driver btfm_slim_driver = {
	...
	.resume = btfm_slim_resume,
	.suspend = btfm_slim_suspend,
	.device_up = btfm_slim_device_up,
	.reset_device = btfm_slim_device_reset,
	.device_down = btfm_slim_device_down,
};
