...
int btfm_slim_chrk_hw_init(struct btfmslim *btfmslim)
{
	...
	if(!btfmslim) return -EINVAL;
	/* Get SB_SLAVE_HW_REV_MSB value*/
	ret = btfm_slim_read(btfmslim , CHRK_SB_SLAVE_HW_REV_MSB,  1, &reg_val, IFD);
	if (ret) {
		BTFMSLIM_ERR("failed to read (%d)", ret);
		goto error;
	}
	BTFMSLIM_INFO("Major Rev: 0x%x, Minor Rev: 0x%x", (reg_val & 0xF0) >>4,  (reg_val & 0x0F));

	/* Get SB_SLAVE_HW_REV_LSB value*/
	ret = btfm_slim_read(btfmslim , CHRK_SB_SLAVE_HW_REV_LSB,  1, &reg_val, IFD);
	if (ret) {
		BTFMSLIM_ERR("failed to read (%d)", ret);
		goto error;
	}
	BTFMSLIM_INFO("Step Rev: 0x%x", reg_val);
...
int btfm_slim_chrk_enable_port(struct btfmslim *btfmslim, u8 port_num,
	u8 rxport, u8 enable)
{
	int ret =0;
	u8 reg_val  =0;
	...
	if(rxport) {
		...
		/* Multiple Channel Setting - only FM Tx will be multiple channel */
		...
		ret = btfm_slim_write(btfmslim ,
			CHRK_SB_PGD_PORT_TX_OR_UR_CFGN(port_num), 1, &reg_val, IFD);
		if (ret) {
			BTFMSLIM_ERR("failed to write (%d)", ret);
			goto error;
		}
