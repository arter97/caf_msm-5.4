	...
	struct snd_soc_dai *dai)
{
	...
	u8  rxport, grp = false, nchan =1;

	BTFMSLIM_INFO("dai->name:%s, dai->id: %d, dai->rate: %d", dai->name, dai->id, dai->rate);
	...
	struct snd_soc_dai *dai)
{
	...
	BTFMSLIM_DBG("dai->name:%s, dai->id: %d, dai->rate: %d", dai->name, dai->id, dai->rate);
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	for ( i=0 ; (rx_chs->port != BTFM_SLIM_PGD_PORT_LAST)&& (i < rx_num); i++, rx_chs++) {
		/* Set Rx Channel number from machine driver and
			get channel handler from slimbus driver
		*/
		rx_chs->ch = * (u8 *)( rx_slot +i );
		ret = slim_query_ch(btfmslim->slim_pgd, rx_chs->ch, &rx_chs->ch_hdl);
		if (ret < 0) {
			BTFMSLIM_ERR("slim_query_ch failure ch#%d - ret[%d]", rx_chs->ch, ret);
			goto error;
		}
		BTFMSLIM_DBG("    %d\t%s\t%d\t%x\t%d\t%x", rx_chs->id, rx_chs->name, rx_chs->port,
			rx_chs->port_hdl, rx_chs->ch, rx_chs->ch_hdl);
	}

	BTFMSLIM_DBG("Tx: id\tname\tport\thdl\tch\tch_hdl");
	for ( i=0 ; (tx_chs->port != BTFM_SLIM_PGD_PORT_LAST)&& (i < tx_num); i++, tx_chs++) {
		/* Set Tx Channel number from machine driver and
			get channel handler from slimbus driver
		*/
		tx_chs->ch = * (u8 *)( tx_slot +i );
		ret = slim_query_ch(btfmslim->slim_pgd, tx_chs->ch, &tx_chs->ch_hdl);
		if (ret < 0) {
			BTFMSLIM_ERR("slim_query_ch failure ch#%d - ret[%d]", tx_chs->ch, ret);
			goto error;
		}
		BTFMSLIM_DBG("    %d\t%s\t%d\t%x\t%d\t%x", tx_chs->id, tx_chs->name, tx_chs->port,
				 ...
				 unsigned int *rx_num, unsigned int *rx_slot)
{
	...
	switch (dai->id) {
		...
		case BTFM_BT_SCO_SLIM_TX:
			if (!tx_slot || !tx_num) {
				BTFMSLIM_ERR("Invalid tx_slot %p or tx_num %p", tx_slot, tx_num);
				return -EINVAL;
			}
			ch = btfmslim->tx_chs;
			if (!ch)
...
next_channel:
	for(i=0;(i < BTFM_SLIM_NUM_CODEC_DAIS)  && (ch->id != BTFM_SLIM_NUM_CODEC_DAIS)
		&& (ch->id != dai->id); ch++, i++);

	if(ch->id== BTFM_SLIM_NUM_CODEC_DAIS || i == BTFM_SLIM_NUM_CODEC_DAIS) {
		BTFMSLIM_ERR("No channel has been allocated for dai (%d) ", dai->id);
		return -EINVAL;
	}

	*(slot + j) = ch->ch;
	BTFMSLIM_DBG("id:%d, port:%d, ch:%d, slot: %d", ch->id, ch->port, ch->ch, *(slot + j) );
...
static struct snd_soc_dai_driver btfmslim_dai[] = {
	{	/* FM Audio data mutiple channel  : FM -> qdsp */
	...
	{	/* Bluetooth SCO voice uplink: bt -> modem */
		...
		.capture = {
			...
			.rates = SNDRV_PCM_RATE_8000 |SNDRV_PCM_RATE_16000 , /* 8 KHz or 16 KHz */
			.formats = SNDRV_PCM_FMTBIT_S16_LE, /* 16 bits */
			.rate_max = 16000,
			.rate_min = 8000,
			.channels_min = 1,
	...
	{	/* Bluetooth SCO voice downlink: modem -> bt or A2DP Playback */
		...
		.playback = {
			...
			.rates = SNDRV_PCM_RATE_8000 |SNDRV_PCM_RATE_16000
				|SNDRV_PCM_RATE_48000, /* 8 KHz or 16 KHz or 48 Khz*/
			.formats = SNDRV_PCM_FMTBIT_S16_LE, /* 16 bits */
			.rate_max = 48000,
			.rate_min = 8000,
			.channels_min = 1,
