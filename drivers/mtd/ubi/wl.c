...
ssize_t ubi_wl_scrub_all(struct ubi_device *ubi, unsigned long long scrub_sqnum)
{
	...
	if (is_ubi_readonly(ubi)) {
		ubi_err(ubi->ubi_num, "Cannot *Initiate* scrub:background thread disabled or readonly!");
		return -EROFS;
	}
	...
	spin_lock(&ubi->wl_lock);
	ubi_msg(ubi->ubi_num, "Scrub triggered sqnum = %llu!", scrub_sqnum);

	if (ubi->scrub_in_progress) {
		ubi_err(ubi->ubi_num,
			"Scrub already in progress, ignoring the trigger");
		spin_unlock(&ubi->wl_lock);
	...
	if (err) {
		ubi_err(ubi->ubi_num, "Failed to start the WL worker err =%d",
			err);
		return err;
	}
	ubi_msg(ubi->ubi_num, "Scheduled %d PEB's for scrubbing!", scrub_count);
	ubi_msg(ubi->ubi_num, "Total PEB's for scrub = %d", total_scrub_count);
	...
	while (atomic_read(&ubi->scrub_work_count) > 0) {
		...
		if (is_ubi_readonly(ubi)) {
			ubi_err(ubi->ubi_num, "Cannot *Complete* scrub:background thread disabled or readonly!");
			return -EROFS;
		}
		wake_up_process(ubi->bgt_thread);
	}
	...
	spin_unlock(&ubi->wl_lock);
	ubi_msg(ubi->ubi_num, "Done scrubbing %d PEB's!", scrub_count);
	return 0;
}
