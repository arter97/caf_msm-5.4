...
static void ipa_vlan_l2tp_msg_free_cb(void *buff, u32 len, u32 type)
{
	if (!buff) {
		IPAERR("Null buffer\n");
		return;
...
static int ipa_send_vlan_l2tp_msg(unsigned long usr_param, uint8_t msg_type)
{
	int retval;
	...
	if (msg_type == ADD_VLAN_IFACE ||
		...
		retval = ipa_send_msg(&msg_meta, vlan_info,
			ipa_vlan_l2tp_msg_free_cb);
		if (retval) {
		...
		retval = ipa_send_msg(&msg_meta, mapping_info,
			ipa_vlan_l2tp_msg_free_cb);
		if (retval) {
			IPAERR("ipa_send_msg failed: %d\n", retval);
			kfree(mapping_info);
...
static long ipa_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IPA_IOC_ADD_VLAN_IFACE:
		if (ipa_send_vlan_l2tp_msg(arg, ADD_VLAN_IFACE)) {
			retval = -EFAULT;
			break;
		}
		break;
	...
	case IPA_IOC_DEL_VLAN_IFACE:
		if (ipa_send_vlan_l2tp_msg(arg, DEL_VLAN_IFACE)) {
			retval = -EFAULT;
			break;
		}
		break;
	...
	case IPA_IOC_ADD_L2TP_VLAN_MAPPING:
		if (ipa_send_vlan_l2tp_msg(arg, ADD_L2TP_VLAN_MAPPING)) {
			retval = -EFAULT;
			break;
		}
		break;
	...
	case IPA_IOC_DEL_L2TP_VLAN_MAPPING:
		if (ipa_send_vlan_l2tp_msg(arg, DEL_L2TP_VLAN_MAPPING)) {
			retval = -EFAULT;
			break;
		}
		break;
