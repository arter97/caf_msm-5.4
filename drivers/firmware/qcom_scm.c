...
enum qcom_scm_reset_type qcom_scm_reset_type;
EXPORT_SYMBOL(qcom_scm_reset_type);
			      ...
			      void *ptr)
{
	...
	if (reboot_mode == REBOOT_WARM && qcom_scm_reset_type == QCOM_SCM_RST_NONE)
		qcom_scm_reset_type = QCOM_SCM_RST_WARM_RESET;

	if (qcom_scm_reset_type != QCOM_SCM_RST_NONE)
		__qcom_scm_reboot(scm->dev, qcom_scm_reset_type);
