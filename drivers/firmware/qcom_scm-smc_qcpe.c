			 ...
			 struct arm_smccc_res *res, const bool atomic)
{
	...
	pr_err("SCM IN [QCPE]: 0x%x, 0x%x, 0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx\n",
...
static void __qcom_scm_qcpe_call_do_quirk(const struct arm_smccc_args *smc,
				  ...
				  struct qcom_scm_desc *desc, const u32 options)
{
	...
	if (atomic) {
		__qcom_scm_qcpe_call_do_quirk(&smc, &res, true);
	} else {
		mutex_lock(&qcom_scm_lock);
		__qcom_scm_qcpe_call_do_quirk(&smc, &res, false);
