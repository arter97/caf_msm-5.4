...
static const struct pinctrl_pin_desc makena_pins[] = {
...
static const struct msm_function makena_functions[] = {
	FUNCTION(gpio),
	FUNCTION(qup12),
 ...
 */
static const struct msm_pingroup makena_groups[] = {
...
static const struct msm_pinctrl_soc_data makena_pinctrl = {
	.pins = makena_pins,
	.npins = ARRAY_SIZE(makena_pins),
	.functions = makena_functions,
	.nfunctions = ARRAY_SIZE(makena_functions),
	.groups = makena_groups,
	.ngroups = ARRAY_SIZE(makena_groups),
	.ngpios = 229,
};

static int makena_pinctrl_probe(struct platform_device *pdev)
{
	return msm_pinctrl_probe(pdev, &makena_pinctrl);
}

static const struct of_device_id makena_pinctrl_of_match[] = {
	{ .compatible = "qcom,makena-pinctrl", },
	{ },
};

static struct platform_driver makena_pinctrl_driver = {
	.driver = {
		.name = "makena-pinctrl",
		.owner = THIS_MODULE,
		.of_match_table = makena_pinctrl_of_match,
	},
	.probe = makena_pinctrl_probe,
	.remove = msm_pinctrl_remove,
};

static int __init makena_pinctrl_init(void)
{
	return platform_driver_register(&makena_pinctrl_driver);
}
arch_initcall(makena_pinctrl_init);

static void __exit makena_pinctrl_exit(void)
{
	platform_driver_unregister(&makena_pinctrl_driver);
}
module_exit(makena_pinctrl_exit);

MODULE_DESCRIPTION("QTI makena pinctrl driver");
MODULE_LICENSE("GPL v2");
MODULE_DEVICE_TABLE(of, makena_pinctrl_of_match);
