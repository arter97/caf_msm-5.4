...
static irqreturn_t wcd9xxx_irq_thread(int irq, void *data)
{
	...
	if (!wcd9xxx_res->codec_bulk_read) {
		pr_err("%s: Codec Bulk Register read callback not supplied\n",
			   __func__);
		return IRQ_NONE;
	}
...
int wcd9xxx_irq_init(struct wcd9xxx_core_resource *wcd9xxx_res)
{
	...
	if (!wcd9xxx_res->codec_reg_write) {
		pr_err("%s: Codec Register write callback not defined\n",
			   __func__);
		return -EINVAL;
	}
	...
	if (ret != 0)
		...
		if (ret == 0) {
			ret = device_init_wakeup(wcd9xxx_res->dev, 1);
			if (ret) {
				dev_err(wcd9xxx_res->dev, "Failed to init device"
					"wakeup : %d\n", ret);
				disable_irq_wake(wcd9xxx_res->irq);
			}
		} else
			dev_err(wcd9xxx_res->dev, "Failed to set wake interrupt on IRQ %d: %d\n"
					, wcd9xxx_res->irq, ret);
		if (ret)
