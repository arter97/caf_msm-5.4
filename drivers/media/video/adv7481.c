...
#define I2C_BLOCK_WRITE_SIZE    32
...
static int adv7481_wr_block_data(struct i2c_client *i2c_client,
		uint8_t reg, uint8_t size, const uint8_t *value)
{
	return i2c_smbus_write_i2c_block_data(i2c_client, reg & 0xFF, size,
			value);
}
...
static int adv7481_set_edid(struct adv7481_state *state)
{
	...
	for (i = 0; i < ADV7481_EDID_SIZE && !ret; i += I2C_BLOCK_WRITE_SIZE)
		ret = adv7481_wr_block_data(state->i2c_edid, i,
			I2C_BLOCK_WRITE_SIZE, &adv7481_default_edid_data[i]);

	data_left = ADV7481_EDID_SIZE % I2C_BLOCK_WRITE_SIZE;
	if (data_left && !ret)
		ret = adv7481_wr_block_data(state->i2c_edid,
			ADV7481_EDID_SIZE - data_left, data_left,
			&adv7481_default_edid_data[
			ADV7481_EDID_SIZE - data_left]);

	return ret;
}

static irqreturn_t adv7481_irq1(int irq, void *dev)
{
...
static void adv7481_irq_delay_work(struct work_struct *work)
{
	...
	int_raw_status = adv7481_rd_byte(state->client,
	...
	while (int_raw_status) {
		if (ADV_REG_GETFIELD(int_raw_status, IO_INTRQ1_RAW)) {
			...
			pr_debug("%s: dev: %d got intrq1_raw\n", __func__,
					state->device_num);
			int_status = adv7481_rd_byte(state->client,
			...
			if (ADV_REG_GETFIELD(int_status, IO_CP_LOCK_CP_ST)) {
				if (ADV_REG_GETFIELD(raw_status, IO_CP_LOCK_CP_RAW)) {
					lock_status = 0;
					pr_debug("%s: set lock_status IO_CP_LOCK_CP_RAW: 0x%x\n",
							__func__, lock_status);
				}
			}
			if (ADV_REG_GETFIELD(int_status, IO_CP_UNLOCK_CP_ST)) {
				if (ADV_REG_GETFIELD(raw_status, IO_CP_UNLOCK_CP_RAW)) {
					lock_status = 1;
					pr_debug("%s: set lock_status IO_CP_UNLOCK_CP_RAW: 0x%x\n",
							__func__, lock_status);
				}
			}
			if (lock_status >= 0) {
						...
						struct adv7481_state *state)
{
	...
	if (gpio_is_valid(pdata->irq1_gpio)) {
		ret = gpio_request(pdata->irq1_gpio, "adv7481_irq1");
		if (ret) {
			pr_err("%s: Failed to request irq1_gpio %d\n",
					__func__, ret);
			goto err_exit;
		}
		...
		if (ret) {
			pr_err("%s: Failed gpio_direction irq1 %d\n",
					__func__, ret);
			goto err_exit;
		}
		...
		if (state->irq) {
			ret = request_irq(state->irq, adv7481_irq1,
					IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
					DRIVER_NAME, state);
			if (ret) {
				pr_err("%s: Failed request_irq 1 %d\n",
						__func__, ret);
				goto err_exit;
			}
		} else {
			pr_err("%s: Failed gpio_to_irq 1 %d\n", __func__, ret);
			ret = -EINVAL;
			goto err_exit;
		}
...
static int adv7481_g_input_status(struct v4l2_subdev *sd, u32 *status)
{
	...
	*status = 0;
	if (ADV7481_IP_HDMI == state->mode) {
		...
		val = adv7481_rd_byte(state->i2c_sdp,
						SDP_RO_MAIN_STATUS1_ADDR);
