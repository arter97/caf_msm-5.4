	...
	struct msm_fd_buf_handle *buf)
{
	...
	if (IS_ERR_OR_NULL(buf->dma_buf)) {
		pr_err("Error: Ion dma get buf failed\n");
		ret = PTR_ERR(buf->dma_buf);
		goto error_get_buf;
	}

	buf->attachment = dma_buf_attach(buf->dma_buf,
				pool->fd_device->iommu_dev);
	if (IS_ERR_OR_NULL(buf->attachment)) {
		pr_err("Error: Ion dma buf attach failed\n");
		ret = PTR_ERR(buf->attachment);
		goto err_attach;
	}

	buf->table = dma_buf_map_attachment(buf->attachment,
					DMA_BIDIRECTIONAL);
	if (IS_ERR_OR_NULL(buf->table)) {
		pr_err("Error:DMA buf map attachment failed\n");
		ret = PTR_ERR(buf->table);
		goto err_map_attach;
	}

	ret = msm_map_dma_buf(buf->dma_buf, buf->table,
			pool->domain_num, 0, SZ_4K, 0, &buf->addr,
			&buf->size, 0, 0);
	if (ret < 0) {
		pr_err("map_dam_buf failed\n");
		goto err_map_buf;
	}
...
err_map_buf:
	dma_buf_unmap_attachment(buf->attachment, buf->table,
		DMA_BIDIRECTIONAL);
err_map_attach:
...
{
	struct msm_fd_mem_pool *pool = buf->pool;
	if (buf->size) {
		msm_unmap_dma_buf(buf->table, pool->domain_num, 0);
		dma_buf_unmap_attachment(buf->attachment, buf->table,
				DMA_BIDIRECTIONAL);
		dma_buf_detach(buf->dma_buf, buf->attachment);
