...
void msm_isp_axi_stream_update(struct vfe_device *vfe_dev, uint8_t input_src)
{
	...
	struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
	for (i = 0; i < MAX_NUM_STREAM; i++) {
		if ((1 << SRC_TO_INTF(axi_data->stream_info[i].stream_src)) &
				input_src) {
...
static int msm_isp_cfg_ping_pong_address(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info, uint32_t pingpong_status,
	uint32_t pingpong_bit, uint8_t sync_dual_vfe)
{
	int i, rc = -1, vfe_id = 0;
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	vfe_dev->hw_info->vfe_ops.axi_ops.reload_wm(vfe_dev, vfe_dev->vfe_base,
			wm_reload_mask);
