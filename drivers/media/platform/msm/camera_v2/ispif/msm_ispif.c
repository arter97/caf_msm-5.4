	...
	struct msm_ispif_params_entry *entry)
{
	...
	for (i = 0; i < entry->num_cids && i < MAX_CID_CH_v2; i++)
		if (entry->right_cids[i] < CID_MAX)
			cids_mask |= (1 << entry->right_cids[i]);
...
{
	int rc = 0, i = 0;
	uint16_t cid_mask, cid_right_mask;
	enum msm_ispif_intftype intftype;
	enum msm_ispif_vfe_intf vfe_intf;
	...
	for (i = 0; i < params->num; i++) {
		...
		if (ispif->csid_version >= CSID_VERSION_V30) {
			...
			if (intftype == PIX0 && params->stereo_enable &&
			    params->entries[i].right_csid < CSID_MAX)
				msm_ispif_select_clk_mux(ispif, PIX1,
					params->entries[i].right_csid,
					vfe_intf);
		}
		...
		if (intftype == PIX0 && params->stereo_enable &&
			...
			msm_ispif_sel_csid_core(ispif, PIX1,
				params->entries[i].right_csid, vfe_intf);
	...
	for (i = 0; i < params->num; i++) {
		...
		for (k = 0; k < params->entries[i].num_cids; k++) {
			...
			if (intf_type == PIX0 && params->stereo_enable &&
			    params->entries[i].right_cids[k] < CID_MAX) {
				cid = params->entries[i].right_cids[k];
				vc = cid / 4;
		...
		enum msm_ispif_vfe_intf vfe_id)
{
	...
	pr_err ("%s;%d Reconfiguring 3D mode for VFE%d", __func__, __LINE__,
			vfe_id);
	reg_data =  0xFFFCFFFC;
