...
static void ts_smdl_transfer(struct smd_ts_apps *ts_app)
{
	...
	while (avail_len > 0) {
		...
		read_len = smd_read_from_cb(ts_app->chan, ts_app->buf_ptr, sizeof(uint64_t));
...
static int ts_smdl_open(struct smd_ts_apps *ts_app)
{
	...
	if (ret == 0) {
		ts_dbg("%s wait for completion failed\n", ts_app->dev_name);
...
static int ts_buf_read(struct smd_ts_apps *ts_app, char *user_buf, size_t count)
{
	...
	length = (ts_app->ts_buf + ts_app->buf_len - ts_app->buf_ptr) * sizeof(uint64_t);
	if (count <= length) {
...
static int smd_ts_apps_init(struct smd_ts_apps *ts_app)
{
	...
	ts_app->ts_buf = kmalloc(sizeof(uint64_t) * ts_app->buf_len, GFP_KERNEL);
	if (!ts_app->ts_buf)
		return -ENOMEM;
...
static struct file_operations ts_diff_fops = {
	.open = ts_diff_open,
	.read = ts_diff_read,
