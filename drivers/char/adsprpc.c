			...
			remote_arg_t *upra)
{
	...
	for (oix = 0; oix < inbufs + outbufs; ++oix) {
		int i = ctx->overps[oix]->raix;
		size_t mlen = ctx->overps[oix]->mend -
...
{
	int err = 0;
	struct fastrpc_mmap *map = NULL, *mapfree = NULL;
	struct hlist_node *n;
