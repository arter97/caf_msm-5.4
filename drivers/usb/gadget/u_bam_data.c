...
struct bam_data_ch_info {
	...
	atomic_t		is_teth_bridge_connected;
};
		...
		gfp_t flags)
{
	...
	d->alloc_rx_reqs = num;
	pr_err("%s(): allocated rx_reqs:%d\n", __func__, d->alloc_rx_reqs);
	return 0;
}
...
static void bam_data_epout_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (queue) {
		...
		if (!usb_bam_get_prod_granted(d->dst_connection_idx)) {
			...
			pr_err("%s(): list_add to rx_idle 1\n", __func__);
			spin_unlock(&port->port_lock);
			return;
		} else
			queue_work(bam_data_wq, &d->write_tobam_w);
	...
	if (bam_mux_rx_fctrl_support &&
		...
		pr_err("%s(): list_add to rx_idle 2\n", __func__);
		list_add_tail(&req->list, &d->rx_idle);
		spin_unlock(&port->port_lock);
		return;
	}
	...
	skb = bam_data_alloc_skb_from_pool(port);
	if (!skb) {
		pr_err("%s(): failed to get skb_from_pool\n", __func__);
		list_add_tail(&req->list, &d->rx_idle);
		spin_unlock(&port->port_lock);
		return;
	}
...
static void bam2bam_data_connect_work(struct work_struct *w)
{
	...
	if (ret) {
		pr_err("%s: _bam_data_start_io failed(%d)", __func__, ret);
		spin_unlock_irqrestore(&port->port_lock, flags);
		return;
	}
	...
	*/
	...
	if (d->trans == USB_GADGET_XPORT_BAM2BAM_IPA) {
		...
		atomic_set(&d->is_teth_bridge_connected, 1);
...
void bam_data_disconnect(struct data_port *gr, u8 port_num)
{
	...
	if (port->port_usb) {
		...
		if (port->port_usb->in && port->port_usb->in->driver_data) {
			...
			pr_err("No of SKB_RX_Q:%d\n", d->rx_skb_q.qlen);
			while ((skb = __skb_dequeue(&d->rx_skb_q))) {
				dev_kfree_skb_any(skb);
				n++;
			}

			pr_err("Freed %d SKB from RX_Q\n", n);
			bam2bam_free_rx_skb_idle_list(port);
			pr_err("allocated_skb:%d freed_skb:%d\n",
					d->total_skb, d->freed_skb);
			pr_err("allocated_rx_req:%d freed_rx_req:%d\n",
	...
	/*
	 * Make sure to perform teth_bridge_disconnect() only if
	 * teth_bridge_init() is performed for different USB functions.
	 */
	if (d->trans == USB_GADGET_XPORT_BAM2BAM_IPA) {
		if (atomic_read(&d->is_teth_bridge_connected)) {
			void *priv;
			...
			pr_debug("%s(): teth_bridge() disconnected\n",
								__func__);
			atomic_set(&d->is_teth_bridge_connected, 0);
