...
int wcnss_prealloc_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(wcnss_skb_allocs); i++) {
		...
		wcnss_skb_allocs[i].skb_ptr =
			dev_alloc_skb(wcnss_skb_allocs[i].size);
		if (wcnss_skb_allocs[i].skb_ptr == NULL)
			return -ENOMEM;
	}
	return 0;
}
...
void wcnss_prealloc_deinit(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(wcnss_skb_allocs); i++)
		dev_kfree_skb_any(wcnss_skb_allocs[i].skb_ptr);
}
...
int wcnss_skb_prealloc_put(struct sk_buff *skb_ptr)
{
	int i = 0;
	unsigned long flags;
	...
	spin_lock_irqsave(&alloc_lock, flags);
	for (i = 0; i < ARRAY_SIZE(wcnss_skb_allocs); i++) {
		if (wcnss_skb_allocs[i].skb_ptr == skb_ptr) {
			wcnss_skb_allocs[i].occupied = 0;
			spin_unlock_irqrestore(&alloc_lock, flags);
			return 1;
		}
