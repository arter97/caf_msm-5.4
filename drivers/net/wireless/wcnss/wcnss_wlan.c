...
void wcnss_pronto_log_debug_regs(void)
{
	...
	reg = readl_relaxed(reg_addr);
	pr_err("%s:  PRONTO_PMU_COM_GDSCR %08x\n",
						__func__, reg);
	reg >>= 31;

	if (!reg) {
		pr_err("%s:  Cannot log, Pronto common SS is power collapsed\n",
				__func__);
		return;
	}
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	pr_err("%s:  AXIM SEL CFG0 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	pr_err("%s:  AXIM SEL CFG1 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	pr_err("%s:  CTRL SEL CFG0 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	pr_err("%s:  CTRL SEL CFG1 testbus %08x\n", __func__, reg);
	...
	pr_err("%s:  PMU_WLAN_AHB_CBCR %08x\n", __func__, reg3);
	...
	if ((reg & PRONTO_PMU_WLAN_BCR_BLK_ARES) ||
		...
		pr_err("%s:  Cannot log, wlan domain is power collapsed\n",
				__func__);
		return;
	}
	...
	pr_err("%s: WLAN_TX_PHY_ABORTS %08x\n", __func__, reg);
	...
	pr_err("%s: WLAN_BRDG_ERR_SOURCE %08x\n", __func__, reg);

	reg = readl_relaxed(penv->wlan_tx_status);
	pr_err("%s: WLAN_TX_STATUS %08x\n", __func__, reg);
