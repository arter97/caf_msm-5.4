...
#define KSZ8851_TX_SPACE (6144 * 2)
#define TX_DMA_BUFFER_SIZE (8192 * 2)
...
static void ks8851_rx_pkts3(struct ks8851_net *ks)
{
	...
	if (rxfifosize < 0) {
		pr_err("ks8851:Memory not available");
		return;
	}
...
static void ks8851_wrpkts3(struct ks8851_net *ks)
{
	...
	for (count = 0; !last && len < KSZ8851_TX_SPACE; count++) {
