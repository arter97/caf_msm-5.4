...
static void eth_ipa_ctx_init(void)
{
	...
	if (eth_ipa_ctx.queue_enabled[IPA_QUEUE_BE]) {
		if (of_property_read_u32(eth_ipa_ctx.ethqos->pdev->dev.of_node,
					 "ipa-dma-rx-desc-cnt",
...
static inline phys_addr_t eth_ipa_queue_type_to_rx_reg_base_ptr_pa(enum ipa_queue_type type)
{
	WARN_ON(!eth_ipa_queue_type_supported(type));
	return eth_ipa_ctx.rx_reg_base_ptr_pa[type];
}

static inline phys_addr_t eth_ipa_queue_type_to_tx_reg_base_ptr_pa(enum ipa_queue_type type)
{
	WARN_ON(!eth_ipa_queue_type_supported(type));
				   ...
				   enum ipa_queue_type type)
{
	...
	if (eth_ipa_ctx.rx_queue[type]->ipa_rx_pa_addrs_base) {
		...
		eth_ipa_ctx.rx_queue[type]->ipa_rx_pa_addrs_base = NULL;
		eth_ipa_ctx.rx_queue[type]->ipa_rx_pa_addrs_base_dmahndl =
		(dma_addr_t)NULL;
				  ...
				  enum ipa_queue_type type)
{
	...
	if (eth_ipa_ctx.tx_queue[type]->ipa_tx_pa_addrs_base) {
		dma_free_coherent
		(GET_MEM_PDEV_DEV,
		 sizeof(dma_addr_t) * eth_ipa_ctx.tx_queue[type]->desc_cnt,
...
static int allocate_ipa_buffer_and_desc(struct qcom_ethqos *ethqos, enum ipa_queue_type type)
{
	int ret = 0;
	unsigned int qinx = 0;
	struct net_device *ndev = dev_get_drvdata(&ethqos->pdev->dev);
	...
	/* TX descriptors */
	eth_ipa_ctx.tx_queue[type]->tx_desc_ptrs[0] =
	 dma_alloc_coherent(GET_MEM_PDEV_DEV,
				    ...
				    enum ipa_queue_type type)
{
	...
	/* update the starting address of desc chain/ring */
	DMA_RDLAR_RGWR(qinx, eth_ipa_ctx.rx_queue[type]->rx_desc_dma_addrs[start_index]);

	ETHQOSDBG("\n");
}

static int ethqos_alloc_ipa_rx_buf(struct qcom_ethqos *ethqos, unsigned int i, gfp_t gfp,
				   enum ipa_queue_type type)
{
	...
	dma_alloc_coherent(GET_MEM_PDEV_DEV, rx_buffer_len,
	...
	if (buff_sgt) {
		ret = dma_get_sgtable(GET_MEM_PDEV_DEV, buff_sgt,
				      eth_ipa_ctx.rx_queue[type]->ipa_buff_va[i],
					ipa_rx_buf_dma_addr, rx_buffer_len);
		if (ret == 0) {
...
static void ethqos_wrapper_tx_descriptor_init_single_q(struct qcom_ethqos *ethqos,
						       enum ipa_queue_type type)
{
	int i;
	...
	for (i = 0; i < eth_ipa_ctx.tx_queue[type]->desc_cnt; i++) {
		...
		ipa_tx_buf_vaddr =
		dma_alloc_coherent(GET_MEM_PDEV_DEV, eth_ipa_queue_type_to_buf_length(type),
...
static void ethqos_wrapper_rx_descriptor_init_single_q(struct qcom_ethqos *ethqos,
						       enum ipa_queue_type type)
{
	int i;
...
static int enable_tx_dma_interrupts(unsigned int QINX, struct qcom_ethqos *ethqos,
				    bool enable_sw_intr)
{
	unsigned int tmp;
...
static int enable_rx_dma_interrupts(unsigned int QINX, struct qcom_ethqos *ethqos,
				    bool enable_sw_intr)
{
	unsigned int tmp;
	unsigned long VARDMA_SR;
...
static void ethqos_ipa_config_queues(struct qcom_ethqos *ethqos)
{
	...
	for (type = 0; type < IPA_QUEUE_MAX; type++) {
		if (eth_ipa_queue_type_enabled(type)) {
			...
			ethqos_wrapper_tx_descriptor_init_single_q(ethqos, type);
			ethqos_wrapper_rx_descriptor_init_single_q(ethqos, type);
		}
	}
}

static void ethqos_configure_ipa_tx_dma_channel(struct qcom_ethqos *ethqos,
						enum ipa_queue_type type)
{
	struct platform_device *pdev = ethqos->pdev;
	struct net_device *dev = platform_get_drvdata(pdev);
...
static void ethqos_configure_ipa_rx_dma_channel(struct qcom_ethqos *ethqos,
						enum ipa_queue_type type)
{
	struct platform_device *pdev = ethqos->pdev;
	struct net_device *dev = platform_get_drvdata(pdev);
			       ...
			       enum ipa_queue_type type)
{
	...
	priv->hw->mac->map_mtl_to_dma(priv->hw,
				      eth_ipa_queue_type_to_rx_queue(type),
				      eth_ipa_queue_type_to_rx_queue(type));
...
static int ethqos_ipa_offload_init(struct qcom_ethqos *pdata, enum ipa_queue_type type)
{
				      ...
				      enum ipa_queue_type type)
{
	...
	if (ret) {
		...
		if (eth_ipa_queue_type_to_send_msg_needed(type))
			eth_ipa_send_msg(ethqos, IPA_PERIPHERAL_DISCONNECT, type);
	}
				  ...
				  enum ipa_queue_type type)
{
	...
	dma_stats->ipa_rx_cur_desc_ptr_indx = GET_RX_DESC_IDX(type, data);

	DMA_RDTP_RPDR_RGRD(eth_ipa_queue_type_to_rx_queue(type), data);
	dma_stats->ipa_rx_tail_ptr_indx = GET_RX_DESC_IDX(type, data);
	...
	DMA_IER_RGRD(eth_ipa_queue_type_to_rx_queue(type),
		     dma_stats->ipa_rx_int_mask);

	DMA_IER_RBUE_UDFRD(eth_ipa_queue_type_to_rx_queue(type),
			   dma_stats->ipa_rx_underflow_irq);
	DMA_IER_ETIE_UDFRD(eth_ipa_queue_type_to_rx_queue(type),
			   dma_stats->ipa_rx_early_trans_comp_irq);

	dma_stats->ipa_tx_desc_ring_base = eth_ipa_ctx.tx_queue[type]->tx_desc_dma_addrs[0];
	dma_stats->ipa_tx_desc_ring_size = eth_ipa_ctx.tx_queue[type]->desc_cnt;
	dma_stats->ipa_tx_buff_ring_base = eth_ipa_ctx.tx_queue[type]->ipa_tx_pa_addrs_base_dmahndl;
	...
	dma_stats->ipa_tx_curr_desc_ptr_indx = GET_TX_DESC_IDX(type, data);

	DMA_TDTP_TPDR_RGRD(eth_ipa_queue_type_to_tx_queue(type), data);
	dma_stats->ipa_tx_tail_ptr_indx = GET_TX_DESC_IDX(type, data);
	...
	DMA_IER_RGRD(eth_ipa_queue_type_to_tx_queue(type),
		     dma_stats->ipa_tx_int_mask);
	DMA_IER_TIE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
			  dma_stats->ipa_tx_transfer_complete_irq);

	DMA_IER_TXSE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
			   dma_stats->ipa_tx_transfer_stopped_irq);

	DMA_IER_TBUE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
			   dma_stats->ipa_tx_underflow_irq);

	DMA_IER_ETIE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
			   dma_stats->ipa_tx_early_trans_cmp_irq);

	DMA_IER_FBEE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
			   dma_stats->ipa_tx_fatal_err_irq);

	DMA_IER_CDEE_UDFRD(eth_ipa_queue_type_to_tx_queue(type),
				      ...
				      enum ipa_queue_type type)
{
	...
	if (eth_ipa_queue_type_to_tx_intr_route(type) == IPA_INTR_ROUTE_HW)
		RGMII_GPIO_CFG_TX_INT_UDFWR(eth_ipa_queue_type_to_tx_queue(type));

	/* Configure interrupt route for ETHQOS RX DMA channel to IPA */
	/* Currently, HW route is supported only for one DMA channel */
	if (eth_ipa_queue_type_to_rx_intr_route(type) == IPA_INTR_ROUTE_HW)
		RGMII_GPIO_CFG_RX_INT_UDFWR(eth_ipa_queue_type_to_rx_queue(type));
		...
		for (type = 0; type < IPA_QUEUE_MAX; type++) {
			if (eth_ipa_queue_type_enabled(type)) {
				if (ethqos_ipa_offload_cleanup(ethqos, type))
					...
					ETHQOSERR("IPA Offload Cleanup Success\n");
			}
			eth_ipa_ctx.ipa_offload_init = false;
		}
	}
				      ...
				      int ev)
{
	...
	case EV_IPA_UC_READY:
			...
			if (eth_ipa_ctx.ipa_ready && !eth_ipa_ctx.ipa_offload_init) {
				for (type = 0; type < IPA_QUEUE_MAX; type++) {
					if (eth_ipa_queue_type_enabled(type)) {
						eth_ipa_ctx.ipa_offload_init =
							!ethqos_ipa_offload_init(eth_ipa_ctx.ethqos,
										 type);
					}
				}
			}
	...
	case EV_DEV_CLOSE:
		...
		proto = eth_ipa_queue_type_to_proto(type);

		for (type = 0; type < IPA_QUEUE_MAX; type++) {
			if (eth_ipa_queue_type_enabled(type)) {
				/* Deregister only for 1st enabled proto */
				if (eth_ipa_ctx.ipa_uc_ready) {
					ipa_uc_offload_dereg_rdyCB(proto);
					break;
				}
			}
		}
