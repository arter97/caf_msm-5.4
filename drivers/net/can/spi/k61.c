...
static netdev_tx_t k61_netdev_start_xmit(struct sk_buff *skb,
					 struct net_device *netdev)
{
...
static struct k61_can *k61_create_priv_data(struct spi_device *spi)
{
	...
	priv_data->tx_buf = devm_kzalloc(dev, XFER_BUFFER_SIZE, GFP_KERNEL);
	priv_data->rx_buf = devm_kzalloc(dev, XFER_BUFFER_SIZE, GFP_KERNEL);
...
static int k61_probe(struct spi_device *spi)
{
	...
	dev_dbg(dev, "k61 Probe created priv_data");
	...
	if (err) {
		dev_err(dev, "Failed to create CAN device: %d\n", err);
		goto cleanup_candev;
	}
