...
static void krait_save_regs(struct arm_pmu *cpu_pmu,
					struct cpupmu_regs *regs)
{
	armv7pmu_save_regs(cpu_pmu, regs);
	regs->pmresr[0] = krait_read_pmresr0();
	regs->pmresr[1] = krait_read_pmresr1();
	regs->pmresr[2] = krait_read_pmresr2();
	krait_pre_vmresr0();
	regs->pmresr[3] = krait_read_vmresr0();
	krait_post_vmresr0();
	asm volatile("mrc p15, 0, %0, c9, c15, 5" : "=r" (regs->pmactlr));
}

static void krait_restore_regs(struct arm_pmu *cpu_pmu,
					struct cpupmu_regs *regs)
{
	armv7pmu_restore_regs(cpu_pmu, regs);
	krait_write_pmresr0(regs->pmresr[0]);
	krait_write_pmresr1(regs->pmresr[1]);
	krait_write_pmresr2(regs->pmresr[2]);
	krait_pre_vmresr0();
	krait_write_vmresr0(regs->pmresr[0]);
	krait_post_vmresr0();
	asm volatile("mcr p15, 0, %0, c9, c15, 5" : : "r" (regs->pmactlr));
}
...
static int armv7_krait_pmu_init(struct arm_pmu *cpu_pmu)
{
	...
	cpu_pmu->save_regs		= krait_save_regs;
	cpu_pmu->restore_regs		= krait_restore_regs;
