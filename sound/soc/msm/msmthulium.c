...
static int msmthulium_spk_control = 1;
...
static const struct soc_enum msmthulium_auxpcm_enum[] = {
...
static struct msmthulium_liquid_dock_dev *msmthulium_liquid_dock_dev;
...
static void msmthulium_liquid_docking_irq_work(struct work_struct *work)
{
	struct msmthulium_liquid_dock_dev *dock_dev =
		container_of(work, struct msmthulium_liquid_dock_dev,
			     irq_work);
...
static irqreturn_t msmthulium_liquid_docking_irq_handler(int irq, void *dev)
{
	struct msmthulium_liquid_dock_dev *dock_dev = dev;
...
static int msmthulium_liquid_init_docking(void)
{
	int ret = 0;
	...
	if (dock_plug_gpio >= 0) {
		msmthulium_liquid_dock_dev =
		 kzalloc(sizeof(*msmthulium_liquid_dock_dev), GFP_KERNEL);
		if (!msmthulium_liquid_dock_dev) {
			pr_err("msmthulium_liquid_dock_dev alloc fail.\n");
			ret = -ENOMEM;
			goto exit;
		}

		msmthulium_liquid_dock_dev->dock_plug_gpio = dock_plug_gpio;

		ret = gpio_request(msmthulium_liquid_dock_dev->dock_plug_gpio,
					   "dock-plug-det-irq");
		if (ret) {
			pr_err("%s:failed request msmthulium_liquid_dock_plug_gpio err = %d\n",
				__func__, ret);
			ret = -EINVAL;
			goto fail_dock_gpio;
		}

		msmthulium_liquid_dock_dev->dock_plug_det =
			gpio_get_value(
				msmthulium_liquid_dock_dev->dock_plug_gpio);
		msmthulium_liquid_dock_dev->dock_plug_irq =
			gpio_to_irq(
				msmthulium_liquid_dock_dev->dock_plug_gpio);

		ret = request_irq(msmthulium_liquid_dock_dev->dock_plug_irq,
				  msmthulium_liquid_docking_irq_handler,
				  dock_plug_irq_flags,
		...
		msmthulium_liquid_dock_dev->audio_sdev.name =
						QC_AUDIO_EXTERNAL_SPK_1_EVENT;

		if (switch_dev_register(
			 &msmthulium_liquid_dock_dev->audio_sdev) < 0) {
		...
		INIT_WORK(
			&msmthulium_liquid_dock_dev->irq_work,
			msmthulium_liquid_docking_irq_work);
	}
	return 0;

fail_switch_dev:
	free_irq(msmthulium_liquid_dock_dev->dock_plug_irq,
				msmthulium_liquid_dock_dev);
fail_dock_gpio:
	gpio_free(msmthulium_liquid_dock_dev->dock_plug_gpio);
exit:
...
static void msmthulium_ext_control(struct snd_soc_codec *codec)
{
	struct snd_soc_dapm_context *dapm = &codec->dapm;

	mutex_lock(&dapm->codec->mutex);
	pr_debug("%s: msmthulium_spk_control = %d", __func__,
		 msmthulium_spk_control);
	if (msmthulium_spk_control == MSMTHULIUM_SPK_ON) {
...
static int msmthulium_get_spk(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msmthulium_spk_control = %d\n",
			 __func__, msmthulium_spk_control);
	ucontrol->value.integer.value[0] = msmthulium_spk_control;
	return 0;
}

static int msmthulium_set_spk(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (msmthulium_spk_control == ucontrol->value.integer.value[0])
		return 0;

	msmthulium_spk_control = ucontrol->value.integer.value[0];
	msmthulium_ext_control(codec);
	return 1;
}

static int msmthulium_ext_us_amp_init(void)
{
	int ret = 0;
...
static void msmthulium_ext_us_amp_enable(u32 on)
{
	if (on)
				    ...
				    struct snd_kcontrol *k, int event)
{
	...
	if (strcmp(w->name, "ultrasound amp")) {
		...
		if (SND_SOC_DAPM_EVENT_ON(event))
			msmthulium_ext_us_amp_enable(1);
		else
			msmthulium_ext_us_amp_enable(0);
	} else {
...
static const struct snd_soc_dapm_widget msmthulium_dapm_widgets[] = {

	SND_SOC_DAPM_SUPPLY("MCLK",  SND_SOC_NOPM, 0, 0,
	msmthulium_mclk_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static int msmthulium_auxpcm_rate_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = msmthulium_auxpcm_rate;
	return 0;
}

static int msmthulium_auxpcm_rate_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	switch (ucontrol->value.integer.value[0]) {
	case 0:
		msmthulium_auxpcm_rate = SAMPLING_RATE_8KHZ;
		break;
	case 1:
		msmthulium_auxpcm_rate = SAMPLING_RATE_16KHZ;
		break;
	default:
		msmthulium_auxpcm_rate = SAMPLING_RATE_8KHZ;
		break;
				      ...
				      struct snd_pcm_hw_params *params)
{
	...
	rate->min = rate->max = msmthulium_auxpcm_rate;
...
static int msmthulium_hdmi_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
					      struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_RATE);
...
static int msmthulium_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static struct snd_soc_ops msmthulium_mi2s_be_ops = {
	.startup = msmthulium_mi2s_snd_startup,
	.shutdown = msmthulium_mi2s_snd_shutdown,
};
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	SOC_ENUM_EXT("Speaker Function", msm_snd_enum[0], msmthulium_get_spk,
			msmthulium_set_spk),
	SOC_ENUM_EXT("SLIM_0_RX Channels", msm_snd_enum[1],
			msm_slim_0_rx_ch_get, msm_slim_0_rx_ch_put),
	SOC_ENUM_EXT("SLIM_0_TX Channels", msm_snd_enum[2],
			msm_slim_0_tx_ch_get, msm_slim_0_tx_ch_put),
	SOC_ENUM_EXT("AUX PCM SampleRate", msmthulium_auxpcm_enum[0],
			msmthulium_auxpcm_rate_get,
			msmthulium_auxpcm_rate_put),
...
static bool msmthulium_swap_gnd_mic(struct snd_soc_codec *codec)
{
...
static int  msmthulium_adsp_state_callback(struct notifier_block *nb,
...
static struct notifier_block adsp_state_notifier_block = {
	.notifier_call = msmthulium_adsp_state_callback,
	.priority = -INT_MAX,
};

static int msmthulium_wcd93xx_codec_up(struct snd_soc_codec *codec)
{
	int err;
...
static int msmthulium_codec_event_cb(struct snd_soc_codec *codec,
				     enum wcd9xxx_codec_event codec_event)
{
	switch (codec_event) {
	case WCD9XXX_CODEC_EVENT_CODEC_UP:
		return msmthulium_wcd93xx_codec_up(codec);
	default:
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	err = msmthulium_liquid_init_docking();
	if (err) {
		pr_err("%s: Thulium init Docking stat IRQ failed (%d)\n",
			__func__, err);
		return err;
	}

	err = msmthulium_ext_us_amp_init();
	if (err) {
		pr_err("%s: Thulium US Emitter GPIO init failed (%d)\n",
			__func__, err);
		return err;
	}
...
static struct snd_soc_ops msmthulium_be_ops = {
	.hw_params = msm_snd_hw_params,
};

static int msmthulium_slimbus_2_hw_params(struct snd_pcm_substream *substream,
					  struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static struct snd_soc_ops msmthulium_slimbus_2_be_ops = {
	.hw_params = msmthulium_slimbus_2_hw_params,
};

/* Digital audio interface glue - connects codec <---> CPU */
static struct snd_soc_dai_link msmthulium_common_dai_links[] = {
	/* FrontEnd DAI Links */
	{
		.name = "MSMTHULIUM Media1",
...
static struct snd_soc_dai_link msmthulium_hdmi_dai_link[] = {
	/* HDMI BACK END DAI Link */
	{
		...
		.be_hw_params_fixup = msmthulium_hdmi_be_hw_params_fixup,
		.ignore_pmdown_time = 1,
		.ignore_suspend = 1,
	},
};
...
static int msmthulium_asoc_machine_probe(struct platform_device *pdev)
{
	...
	pdata = devm_kzalloc(&pdev->dev,
			sizeof(struct msmthulium_asoc_mach_data), GFP_KERNEL);
	if (!pdata) {
		dev_err(&pdev->dev, "Can't allocate msmthulium_asoc_mach_data\n");
		return -ENOMEM;
	}
	...
	if (pdata->us_euro_gpio < 0) {
		...
		mbhc_cfg.swap_gnd_mic = msmthulium_swap_gnd_mic;
	}

	ret = msmthulium_prepare_us_euro(card);
	if (ret)
...
static int msmthulium_asoc_machine_remove(struct platform_device *pdev)
{
	struct snd_soc_card *card = platform_get_drvdata(pdev);
	struct msmthulium_asoc_mach_data *pdata =
				snd_soc_card_get_drvdata(card);
	...
	if (msmthulium_liquid_dock_dev != NULL) {
		switch_dev_unregister(&msmthulium_liquid_dock_dev->audio_sdev);

		if (msmthulium_liquid_dock_dev->dock_plug_irq)
			free_irq(msmthulium_liquid_dock_dev->dock_plug_irq,
				 msmthulium_liquid_dock_dev);

		if (msmthulium_liquid_dock_dev->dock_plug_gpio)
			gpio_free(msmthulium_liquid_dock_dev->dock_plug_gpio);
...
static struct platform_driver msmthulium_asoc_machine_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = msmthulium_asoc_machine_of_match,
	},
	.probe = msmthulium_asoc_machine_probe,
	.remove = msmthulium_asoc_machine_remove,
};
module_platform_driver(msmthulium_asoc_machine_driver);

MODULE_DESCRIPTION("ALSA SoC msm");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" DRV_NAME);
MODULE_DEVICE_TABLE(of, msmthulium_asoc_machine_of_match);
