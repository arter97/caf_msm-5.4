...
#define DRV_NAME "msmsamarium-asoc-tapan"
...
struct msmsamarium_asoc_mach_data {
	int mclk_gpio;
	u32 mclk_freq;
	int us_euro_gpio;
};
					...
					bool dapm)
{
	...
	if (enable) {
		if (!codec_clk) {
			dev_err(codec->dev, "%s: did not get Tapan MCLK\n",
				__func__);
			ret = -EINVAL;
			goto exit;
		}
...
static int msmsamarium_mclk_event(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_widget msmsamarium_dapm_widgets[] = {

	SND_SOC_DAPM_SUPPLY("MCLK",  SND_SOC_NOPM, 0, 0,
	msmsamarium_mclk_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("PROXY_RX Channels", msm_snd_enum[5],
...
static int  msmsamarium_adsp_state_callback(struct notifier_block *nb,
...
static struct notifier_block adsp_state_notifier_block = {
	.notifier_call = msmsamarium_adsp_state_callback,
	.priority = -INT_MAX,
};

static int msmsamarium_tapan_codec_up(struct snd_soc_codec *codec)
{
	int err;
...
static int msmsamarium_tapan_event_cb(struct snd_soc_codec *codec,
	enum wcd9xxx_codec_event codec_event)
{
	switch (codec_event) {
	case WCD9XXX_CODEC_EVENT_CODEC_UP:
		return msmsamarium_tapan_codec_up(codec);
	default:
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_new_controls(dapm, msmsamarium_dapm_widgets,
				ARRAY_SIZE(msmsamarium_dapm_widgets));
	...
	tapan_event_register(msmsamarium_tapan_event_cb, rtd->codec);
	return 0;
out:
	return err;
}

static int msmsamarium_snd_startup(struct snd_pcm_substream *substream)
{
...
static void msmsamarium_snd_shudown(struct snd_pcm_substream *substream)
{
	pr_debug("%s(): substream = %s stream = %d\n", __func__,
		 substream->name, substream->stream);
...
static struct snd_soc_ops msmsamarium_be_ops = {
	.startup = msmsamarium_snd_startup,
	.hw_params = msm_snd_hw_params,
	.shutdown = msmsamarium_snd_shudown,
};


/* Digital audio *interface glue - connects codec <---> CPU */
static struct snd_soc_dai_link msmsamarium_dai_links[] = {
	/* FrontEnd DAI Links */
	{
		.name = "MSMSAMARIUM Media1",
...
static struct snd_soc_dai_link msmsamarium_dai_links[
					 ARRAY_SIZE(msmsamarium_dai_links)];

struct snd_soc_card snd_soc_card_msmsamarium = {
	.name		= "msmsamarium-tapan-snd-card",
	.dai_link       = msmsamarium_dai_links,
	.num_links      = ARRAY_SIZE(msmsamarium_dai_links),
};

static int msmsamarium_prepare_codec_mclk(struct snd_soc_card *card)
{
	struct msmsamarium_asoc_mach_data *pdata =
...
static bool msmsamarium_swap_gnd_mic(struct snd_soc_codec *codec)
{
...
static int msmsamarium_setup_hs_jack(struct platform_device *pdev,
		struct msmsamarium_asoc_mach_data *pdata)
{
	int rc;
	...
	if (pdata->us_euro_gpio < 0) {
		...
		if (rc) {
			...
			mbhc_cfg.swap_gnd_mic = msmsamarium_swap_gnd_mic;
		}
	}
	return 0;
}

static int msmsamarium_asoc_machine_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &snd_soc_card_msmsamarium;
	struct msmsamarium_asoc_mach_data *pdata;
	int ret;
	...
	pdata = devm_kzalloc(&pdev->dev,
			sizeof(struct msmsamarium_asoc_mach_data), GFP_KERNEL);
	if (!pdata) {
		dev_err(&pdev->dev, "Can't allocate msmsamarium_asoc_mach_data\n");
		return -ENOMEM;
		goto err;
	}
	...
	ret = msmsamarium_prepare_codec_mclk(card);
	if (ret)
		goto err1;
	...
	msmsamarium_setup_hs_jack(pdev, pdata);
	return 0;
...
static int msmsamarium_asoc_machine_remove(struct platform_device *pdev)
{
...
static const struct of_device_id msmsamarium_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,msmsamarium-audio-tapan", },
	{},
};

static struct platform_driver msmsamarium_asoc_machine_driver = {
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = msmsamarium_asoc_machine_of_match,
	},
	.probe = msmsamarium_asoc_machine_probe,
	.remove = msmsamarium_asoc_machine_remove,
};
module_platform_driver(msmsamarium_asoc_machine_driver);

MODULE_DESCRIPTION("ALSA SoC msm");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" DRV_NAME);
MODULE_DEVICE_TABLE(of, msmsamarium_asoc_machine_of_match);
