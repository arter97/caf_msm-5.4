...
#define COMPR_PLAYBACK_MIN_FRAGMENT_SIZE (4 * 1024)
...
static int msm_compr_configure_dsp(struct snd_compr_stream *cstream)
{
	...
	if (prtd->compr_passthr) {
		ret = q6asm_open_write_compressed(ac, prtd->codec);
		if (ret < 0) {
			pr_err("%s: Session out open failed with  err[%d]\n",
				__func__, ret);
			return ret;
		}
...
static int validate_codec_compr(__u32 codec_id)
{
	int i;
