...
static int msm_compr_playback_free(struct snd_compr_stream *cstream)
{
	...
	struct msm_compr_pdata *pdata;
	struct audio_client *ac;
	int dir = IN, ret = 0, stream_id, port_id = 0;
	int copp_idx = 0;
	bool tmp = false;
	unsigned long flags;
	uint32_t stream_index;
			...
			const char *name_suffix, int session_type, int channels)
{
	...
	if (channels >= 0) {
		...
		kctl->private_value = (rtd->dai_link->be_id) | (channels << 16);
	} else {
		snprintf(mixer_name, ctl_len, "%s %d %s",
			name_prefix, rtd->pcm->device, name_suffix);
		kctl->private_value = (rtd->dai_link->be_id);
	}
...
static int msm_compr_add_channel_mixer_controls(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!pdata->chmixer_pspd[rtd->dai_link->be_id]) {
		pdata->chmixer_pspd[rtd->dai_link->be_id] =
...
fail:
	kfree(pdata->chmixer_pspd[rtd->dai_link->be_id]);
	pdata->chmixer_pspd[rtd->dai_link->be_id] = NULL;
	return ret;
}
