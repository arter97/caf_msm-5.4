...
#define SSR_NOT_REQUESTED            0
#define SSR_REQUESTED                1
					      ...
					      int event)
{
	...
	dev_dbg(codec->dev,
		...
		__func__, codec->num_dai, w->sname,
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_unlock(&tavil_p->codec_mutex);
	snd_soc_dapm_mixer_update_power(widget, kcontrol, wval);
			       ...
			       void *priv)
{
	...
	static int ssr_request = SSR_NOT_REQUESTED;

	if (value == SUBSYS_BEFORE_SHUTDOWN) {
		ssr_request = SSR_REQUESTED;
		...
		if (ssr_request == SSR_REQUESTED) {
			ssr_request = SSR_NOT_REQUESTED;
